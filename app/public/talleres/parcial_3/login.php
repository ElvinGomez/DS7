...
<div class="min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
  <div class="max-w-md w-full space-y-8">
    ...
    <form class="mt-8 space-y-6" method="POST" action="<?php echo $_SERVER['PHP_SELF'];?>">
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $result = $database->select($query, [$username, $password]);

        if($result) {
          $_SESSION["codusuario"] = $result[0];
          $_SESSION["usuario"] = $result[1];
          $_SESSION["contrasena"] = $result[2];
          $_SESSION["nombre"] = $result[3];
          $_SESSION["apellido"] = $result[4];
          $_SESSION["profile_picture"] = $result[5];
          header("Location: index.php");
        } else {
          echo '
